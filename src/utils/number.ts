...
/**
 * Formats ETH balance with appropriate precision
 * - Very small amounts (< 0.0001): Shows up to 8 decimal places
 * - Small amounts (0.0001 - 1): Shows up to 6 decimal places
 * - Medium amounts (1 - 100): Shows up to 4 decimal places
 * - Large amounts (>= 100): Shows up to 2 decimal places
 * @param {number | string} ethAmount - The ETH amount to format
 * @returns {string} The formatted ETH amount
 * @example
 * formatEthBalance(0.00001234) // returns "0.00001234"
 * formatEthBalance(0.123456) // returns "0.123456"
 * formatEthBalance(1.23456789) // returns "1.2346"
 * formatEthBalance(123.456789) // returns "123.46"
 * formatEthBalance("0.0048") // returns "0.0048"
 */
export const formatEthBalance = (ethAmount: number | string): string => {
  ...
  // Very small amounts: show up to 8 decimals
  if (amount < 0.0001) {
  ...
  // Medium amounts: show up to 4 decimals
  if (amount < 100) {
    return amount.toFixed(4).replace(/\.?0+$/, "");
  }

  // Large amounts: show up to 2 decimals
