...
import type { AgentBalanceLeaderboard, AgentBalanceLeaderboardResponse } from "@/interfaces/agent";
...
export const WaitingList = async () => {
  ...
  try {
    const response: any = await getAgentBalanceLeaderboard({ page: 1, limit: 7 });

    const leaderboardData: AgentBalanceLeaderboard[] = 
      response?.data?.data || [];

    agents = leaderboardData.map((agent: AgentBalanceLeaderboard, index: number) => {
